...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    String prevInfo, newInfo, globalInfo;
    Button bt;
    ...
    private Runnable mCheck = new Runnable() {
        ...
        public void run() {
            ...
            //code to check updates
            if (globalPosition > 0) {
                checkerCrawler(globalPosition);
            }
            if (prevInfo.contains("Aucune coupure pour cette commune ")) {
                prevInfo = newInfo;
            } else {

                if (!(newInfo.equals(prevInfo))) {

                    doNotify("Nouveau Perturbation", newInfo);
                    prevInfo = newInfo;
            }

            }
            // doNotify("hey", globalInfo);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, final int position, long id) {
        checkerCrawler(position);
        globalPosition = position; //nchedo position ta3 commune li kheyerha talya
        prevInfo = globalInfo;
    }
    ...
    public void checkerCrawler(final int position){
        try {
